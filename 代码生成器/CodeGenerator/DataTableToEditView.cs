...
{
    public class DataTableToEditView
    {
        public static string Get(DbTable table, string ns, string an)
        {
            ...
            sb.Append("@{" + Environment.NewLine);
            sb.Append("    ViewData[\"Title\"] = \"" + className + "编辑\";" + Environment.NewLine);
            sb.Append("}" + Environment.NewLine);
            sb.Append(Environment.NewLine);
            sb.Append("<form asp-action=\"Edit\">" + Environment.NewLine);
            sb.Append("    <input type=\"hidden\" asp-for=\"Id\">" + Environment.NewLine);
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (!cols.Contains(dr.name))
                {
                    ...
                    else
                    {
                        if (dr.name.EndsWith("Id"))
                        {
                            sb.Append("                        <select asp-for=\"" + dr.name + "\" isvalid=\"yes\" checkexpession=\"NotNull\">" + Environment.NewLine);
                            sb.Append("                            <option value=\"\">请选择</option>" + Environment.NewLine);
                            sb.Append("                        </select>" + Environment.NewLine);
                        }
                        else
