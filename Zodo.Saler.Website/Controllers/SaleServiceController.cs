...
namespace Zodo.Saler.Website.Controllers
{
    ...
    public class SaleServiceController : BaseController
    {
        ...
        public SaleServiceController(IHostingEnvironment hostingEnvironment)
        {
        ...
        public JsonResult Get(MonthReportSearchParam param, int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                ...
                return Json(ResultUtil.List<MonthReportSaleServiceViewDto>(list));
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Edit(int id, IFormCollection collection)
        {
            ...
            var result = service.SaleServiceSubmit(entity, AppUser);
            return Json(result);
        }
        #endregion
        ...
        public IActionResult Template(int year, int month)
        {
            try
            {
                ...
                using (ExcelPackage package = new ExcelPackage(new System.IO.FileInfo(savePath)))
                {
                    ...
                    workSheet.Cells[rowIndex, 8].Value = "业绩";
                    workSheet.Cells[rowIndex, 9].Value = "账面毛利";
                    ...
                    foreach (var report in list)
                    {
                        ...
                        workSheet.Cells[rowIndex, 8].Value = report.YJ;
                        workSheet.Cells[rowIndex, 9].Value = report.ZMML;
                        ...
                        workSheet.Cells[rowIndex, 1, rowIndex, 9].Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
        ...
        {
            ViewBag.ReturnUrl = $"/SaleService/Index?year={year}&month={month}";
            ViewBag.Year = year;
            ViewBag.Month = month;
            return View();
        }
        ...
        {
            ViewBag.ReturnUrl = $"/SaleService/Index?year={year}&month={month}";
            ViewBag.ImportUrl = $"/SaleService/Import?year={year}&month={month}";
            ...
            var source = service.GetSaleServiceList(param);
            if (source.Count() == 0)
            {
                ...
                using (ExcelPackage package = new ExcelPackage(file))
                {
                    ...
                    void Convert(ExcelWorksheet sheet, int idx)
                    {
                        try
                        {
                            ...
                            decimal.TryParse(sheet.Cells[idx, 8].Value.ToString(), out yj);
                            decimal.TryParse(sheet.Cells[idx, 9].Value.ToString(), out zmml);
                            ...
                            {
                                var submitResult = service.SaleServiceSubmit(entity, AppUser);
