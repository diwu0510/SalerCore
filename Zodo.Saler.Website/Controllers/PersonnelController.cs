...
{
    [Authorize(Roles = "admin, 人事部")]
    public class PersonnelController : BaseController
    {
        private readonly MonthReportService service = new MonthReportService();
        private readonly IHostingEnvironment _hostingEnvironment;

        public PersonnelController(IHostingEnvironment hostingEnvironment)
        {
        ...
        public JsonResult Get(MonthReportSearchParam param, int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                var list = service.GetPersonnelList(param);
                return Json(ResultUtil.List<MonthReportPersonnelViewDto>(list));
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Edit(int id, IFormCollection collection, bool isAsync)
        {
            var entity = new MonthReportEntity();
            TryUpdateModelAsync(entity);

            var result = service.PersonnelSubmit(entity, AppUser, isAsync);
            return Json(result);
        }
        #endregion
        ...
        public IActionResult Template(int year, int month)
        {
            try
            {
                ...
                string fileName = $"人事部{DateTime.Today.ToString("yyyyMM")}.xlsx";
        ...
        {
            ViewBag.ReturnUrl = $"/Personnel/Index?year={year}&month={month}";
            ViewBag.Year = year;
            ViewBag.Month = month;
            return View();
        }
        ...
        {
            ViewBag.ReturnUrl = $"/Personnel/Index?year={year}&month={month}";
            ViewBag.ImportUrl = $"/Personnel/Import?year={year}&month={month}";
            ...
            var source = service.GetPersonnelList(param);
            if (source.Count() == 0)
            {
                ...
                using (ExcelPackage package = new ExcelPackage(file))
                {
                    ...
                    void Convert(ExcelWorksheet sheet, int idx)
                    {
                        try
                        {
                            ...
                            {
                                var submitResult = service.PersonnelSubmit(entity, AppUser, false);
